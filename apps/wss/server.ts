...
import { ConnectionMetadata } from "./src/metadata";
import { createRouter } from "@ws-kit/zod";
import { roomRouter } from "./src/features/room";


const router = createRouter<ConnectionMetadata>();
router.merge(roomRouter);
router.onClose((ctx) => {
  for (const topic of ctx.ws.data.subscriptions) {
    ctx.ws.unsubscribe(topic);
    console.log(`Unsubscribed from ${topic}`);
  }
  ctx.ws.data.subscriptions.clear();
});
...
serve(router, {
  ...
  authenticate(req) {
    ...
    return { userId } as ConnectionMetadata; // client id is supposed to be automatically generated by ws-kit
  },
  onOpen(ctx) {
    console.log(`Client ${ctx.ws.data.userId} connected`);
    ctx.ws.data.subscriptions = new Set<string>();
  },
});
